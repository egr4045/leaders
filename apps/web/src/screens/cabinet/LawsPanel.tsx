...
export function LawsPanel({ you }: { you: PrivateCountryView }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {available.length === 0 ? (
        <div className="py-12 text-center text-sm text-slate-500">Нет доступных законов для принятия</div>
      ) : (
        <div className="flex flex-col gap-2">
          <div className="text-xs text-slate-400 mb-1">
            Принятие закона стоит денег и влияния, а также может требовать определённого числа министров.
          </div>
          {available.map((law) => {
